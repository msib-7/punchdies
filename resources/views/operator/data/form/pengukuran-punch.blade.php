...
@section('main-content')
                        ...
                        {{-- Modal Add Referensi Pengukuran --}}
                        <div class="modal fade" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel2" aria-hidden="true" id="modal_add_referensi">
                            <div class="modal-dialog modal-dialog-centered modal-sm">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h6 class="modal-title">Input Master Punch Length</h6>
                                    </div>
                                    <div class="modal-body">
                                        <div class="d-flex flex-column">
                                            <div class="input-group mb-4">
                                                <input type="text" class="form-control" aria-label="Length" id="master_punch" name="master_punch" oninput="this.value = this.value.replace(/[^0-9.]/g, '');" inputmode="numeric" placeholder="133.68" required/>
                                                <span class="input-group-text"> mm </span>
                                                <small class="my-2">
                                                    <i style="color: red;">* </i><i>gunakan titik (.) sebagai desimal</i>
                                                </small>
                                            </div>
                                            <div class="d-flex justify-content-end">
                                                <button type="submit" class="btn btn-primary btn-sm" onclick="saveMasterRef()">Save changes</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <script>    
                            ...
                            function checkMasterRef(){
                                $.ajax({
                                    url: "{{ url('cek-master-referensi') }}",
                                    type: "POST",
                                    data: {
                                        "_token": "{{ csrf_token() }}",
                                        "punch_id": "{{ $labelPunch->punch_id }}",
                                        "masaPengukuran": `<?= $labelPunch->masa_pengukuran; ?>`
                                    },
                                });
                            };

                            function saveMasterRef() {
                                var master_punch = document.getElementById('master_punch').value;
                                $.ajax({
                                    url: "{{url('save-master-referensi')}}",
                                    type: "POST",
                                    data: {
                                        "_token": "{{ csrf_token() }}",
                                        "master_punch": master_punch,
                                        "punch_id": "{{ $labelPunch->punch_id }}",
                                        "masaPengukuran": `<?= $labelPunch->masa_pengukuran; ?>`
                                    },
                                    beforeSend: function() {
